...
def single_pauli(
    q, j, axis
):  # q: number of qubits, target jth qubit, axis of the Pauli matrix (x->0, y->1, z->2)
    product = [qeye(2)] * q
...
def simulation(q, T, omegas, delta_0, delta_s, V, noise=0):
    ...
    expval_list = np.array(
        [expect(measurement[j], result_t.states[-1]) for j in range(q)]
    )
...
class Encoder(nn.Module):
    def __init__(self, q, Q_size, layers, omega_max, delta_max, T):
        ...
        self.input_size = int(
            Q_size * (Q_size + 1) / 2
        )  # dimension of the input QUBO data
        self.layers = layers
        ...
        for i in range(1, layers):
            if i < layers - 1:
                self.hidden.append(
                    nn.Linear(
                        3 * self.q + self.input_size, 3 * self.q + self.input_size
                    )
                )
            else:
    ...
    def forward(self, x):
        ...
        omega_out = (out[: self.q] * self.omega_max).reshape(
            self.q, 1
        )  # Ω domain [0,omega_max]
        delta_out_0 = (
            -self.delta_max / 2 + out[self.q : 2 * self.q] * self.delta_max
        ).reshape(self.q, 1)  # Δ(0) domain [-delta_max/2,delta_max/2]
        delta_out_s = (
            -self.delta_max / (2 * self.T) + out[2 * self.q :] * self.delta_max / self.T
        ).reshape(self.q, 1)  # s domain [-delta_max/2T, delta_max/2T]
...
class Quantum(torch.autograd.Function):
    ...
    def forward(ctx, input, q, T, V, noise):
        ...
        return torch.tensor(
            simulation(q, T, omegas, delta_0, delta_s, V, noise),
            dtype=torch.float32,
            requires_grad=True,
        )
    ...
    def backward(ctx, grad_output):
        ...
        return (
            torch.cat((loss_omegas, loss_delta_0, loss_delta_s), dim=1),
            None,
            None,
            None,
            None,
...
def QCED_solve(
    ...
    # take the upper-triangular elements of the QUBO matrix as input
    ...
    binary_solution = [
        np.array([0 if v < 0.5 else 1 for v in solution[i]]) for i in range(num_epochs)
    ]
...
def plot_learning_curve(result, figsize=(6, 4), logx=False, logy=False):
    ...
    if logy:
        if result[0][-1] < 0:
            raise ValueError(
                "Negative values found in the result. Try percentage error loss instead or disable logy."
            )
        else:
            plt.yscale("log")
    plt.show()
